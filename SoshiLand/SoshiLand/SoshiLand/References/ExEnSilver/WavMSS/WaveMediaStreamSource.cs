namespace WaveMSS
{
	...
	public class WaveMediaStreamSource : MediaStreamSource, IDisposable
	{
		...
		public void Dispose()
		{
			// TODO: This function runs on the main thread, not the Async thread!
			//       It needs to dispose the `wavParser` (which will also close the `stream`).
			//       Therefore all access to `wavPaser` and `stream` needs to be synchronised and checked for closure
			//
			// Bit busy right now to implement all that... maybe later
			// At the moment it depends on Silverlight calling CloseMedia (better than nothing)
		}
